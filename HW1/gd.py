...
def ada(X, Y, w, eta, iteration, lambdaL2):
    ...
    for i in range(iteration):
        ...
        grad = np.dot(X.T, loss) / len(X) + lambdaL2 * w  # 矩阵求解每个w（包括b）的值
        s_grad += grad ** 2
        ada = np.sqrt(s_grad)
        w = w - eta * grad / ada
    return w, list_cost
...
for r in row:
    if n_row % 18 == 0:
        test_x.append([])
        for i in range(2, 11):
            test_x[n_row // 18].append(float(r[i]))  # //向下取整
    else:
        for i in range(2, 11):
            if r[i] != "NR":
...
# add bias
test_x = np.concatenate((np.ones((test_x.shape[0], 1)), test_x), axis=1)  # 合并，水平合并
